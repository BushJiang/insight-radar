...
export default function RecommendationsPageClient({ initialProjects }: RecommendationsPageClientProps) {
  ...
  const projects = initialProjects.slice(0, recommendationLimit)

  function handleGenerated(recommendation: RecommendationExplanation) {
    setRecommendations([recommendation])
    writeTransientRecommendationFormState({ recommendations: [recommendation] })
  }
      ...
      <main className="space-y-6">
        ...
        <section className="space-y-4">
          ...
          {recommendations.map((recommendation) => (
            <RecommendationExplanationCard key={recommendation.id} recommendation={recommendation} projects={initialProjects} />
          ))}
        </section>
      </main>
    </AppShell>
