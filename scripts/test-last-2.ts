...
const ids = ['1156956890', '1169573829']

for (const id of ids) {
  const project = await getProjectByRepositoryId(id)
  if (!project) { console.log(`${id}: 不存在`); continue }
  ...
  console.log(`${project.fullName}: prompt=${fullPrompt.length}chars, text_len=${result.text?.length ?? 0}, text=${JSON.stringify(result.text?.slice(0, 100))}, finishReason=${result.finishReason}`)
}
