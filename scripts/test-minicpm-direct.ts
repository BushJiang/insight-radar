...
const project = await getProjectByRepositoryId('OpenBMB/MiniCPM'.toLowerCase())
const altProject = await getProjectByRepositoryId('822463486') // try numeric id

console.log('按 fullName 查询:', project ? '找到' : '不存在')
console.log('按 numeric id 查询:', altProject ? altProject.fullName : '不存在')
